...
public interface PackageActionCompletionRepository extends JpaRepository<PackageActionCompletion, UUID> {

    // 역할별 패키지 조회 - 이 단계에서 이미 완료된 행동 전체 (진행률 계산용)
    List<PackageActionCompletion> findByHandoverStage_StageId(UUID stageId);

    // 행동 완료 처리 - 같은 행동을 다시 완료 요청했는지 확인
    Optional<PackageActionCompletion> findByHandoverStage_StageIdAndItemId(UUID stageId, UUID itemId);
