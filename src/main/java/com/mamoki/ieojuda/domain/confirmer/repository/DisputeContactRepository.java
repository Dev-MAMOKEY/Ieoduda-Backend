...
public interface DisputeContactRepository extends JpaRepository<DisputeContact, UUID> {
    ...
    Optional<DisputeContact> findFirstByPlan_PlanIdOrderByContactIdDesc(UUID planId);

    // 계정 삭제 시 이 계획에 등록된 이의 제기 연락처를 전부 지우기 위한 조회
    List<DisputeContact> findByPlan_PlanId(UUID planId);
}
