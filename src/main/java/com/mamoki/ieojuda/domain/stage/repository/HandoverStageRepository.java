...
public interface HandoverStageRepository extends JpaRepository<HandoverStage, UUID> {

    // 계정 삭제 시 이 계획의 인계 단계를 전부 지우기 위한 조회 (email_logs보다 나중, role_assignees/release_cases보다 먼저 지워야 함)
    List<HandoverStage> findByPlan_PlanId(UUID planId);
    ...
    Optional<HandoverStage> findFirstByReleaseCase_CaseIdAndStageOrderGreaterThanOrderByStageOrderAsc(
            UUID caseId, Integer stageOrder);
    ...
    Optional<HandoverStage> findByIdForUpdate(@Param("stageId") UUID stageId);
}
