...
public interface ReleaseCaseRepository extends JpaRepository<ReleaseCase, UUID> {

    // 계획당 진행 중인 사건은 하나뿐 - 작성자 본인용 대기상태 조회, 사망 신고 시 중복 생성 방지에 사용
    Optional<ReleaseCase> findFirstByPlan_PlanIdOrderByCaseIdDesc(UUID planId);
    ...
    Optional<ReleaseCase> findByIdForUpdate(@Param("caseId") UUID caseId);

    // 계정 삭제 시 이 계획에 쌓인 사건(취소/완료된 과거 이력 포함)을 전부 지우기 위한 조회
    List<ReleaseCase> findByPlan_PlanId(UUID planId);
}
